...
MadRuler::MadRuler(wxWindow * parent, int width, wxString& fontname, int fontsize)
	:m_Height(0), m_Width(width), m_Unit(0), m_Step(0), m_Parent(parent)
{
	m_Font = wxTheFontList->FindOrCreateFont( fontsize, wxFONTFAMILY_DEFAULT, wxFONTSTYLE_NORMAL, wxFONTWEIGHT_NORMAL, false, fontname );
	wxSize size = m_Font->GetPixelSize();
	m_Height = size.GetHeight();
	m_Height += m_Height/2;
	m_Unit = size.GetWidth();
	m_Step = 10;
}
...
void MadRuler::Draw()
{
	if(m_Width)
	{
		...
		dc.DrawLine( 0, m_Height-1, m_Width, m_Height );
	}
}
	...
	: MadEditSuperClass( parent, id, pos, size, style ) //wxWANTS_CHARS|wxSIMPLE_BORDER)//|style|wxTAB_TRAVERSAL)
{
	...
	m_Ruler = new MadRuler(this, 0, wxString("Courier New"), 8);
...
void MadEdit::OnSize( wxSizeEvent &evt )
{
	...
	else
	{
		...
		if( ( m_ClientHeight = h - m_Ruler->m_Height - m_HSBHeight + 1 ) <= 0 ) m_ClientHeight = 10;

		m_Ruler->m_Width = m_ClientWidth;
...
void MadEdit::OnPaint( wxPaintEvent &evt )
{
	...
	if( InPrinting() )
	{
		...
		dc.Blit( 0, ypos, m_old_ClientWidth, m_old_ClientHeight, &memdc, 0, ypos );
	}
	else
	{
		if( m_EditMode != emHexMode )
		{
			...
			if( m_RepaintAll )
			{
				...
				memdc.DrawRectangle( 0, ypos, m_ClientWidth, m_ClientHeight );
				// paint rows
				wxRect rect( 0, ypos, m_ClientWidth, m_ClientHeight );
						...
						x < maxright && x + w > minleft )
				{
					...
					if( w > 0 )
					{
						if( bPaintMark == false )
						{
							...
							markdc.Blit( 0, ypos, m_ClientWidth, m_ClientHeight, &memdc, 0, ypos );
							bPaintMark = true;
						}

						wxDC *dc = &markdc;
						INVERT_RECT( dc, x, y+ypos, w, h );
					}
				}
					...
					&& m_CaretPos.rowid < m_TopRow + m_VisibleRowCount )
			{
				...
				markdc.DrawRectangle( x, y+ypos, w, h );
			}
...
#endif
				dc.Blit( 0, ypos, m_ClientWidth, m_ClientHeight, &markdc, 0, ypos );
				m_LastPaintBitmap = 1;
			}
			else
			{
				dc.Blit( 0, ypos, m_ClientWidth, m_ClientHeight, &memdc, 0, ypos );
				m_LastPaintBitmap = 0;
			}
		}
		else // HexMode
